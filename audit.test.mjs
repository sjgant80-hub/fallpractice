// audit.test.mjs — the layout must reproduce what fallpractice has ALREADY written.
...
console.log('\n=== §1 · ⚑ THE HISTORICAL BYTES, UNCHANGED ===');
{
  ...
  const legacy = async (prev, e, i, sha) => prev + (await sha(JSON.stringify(e.payload || {}))) + e.ts + e.i;
  const e = {"i":4,"ts":1723550000006,"action":"client.updated","payload":{"clientId":"c1"}};
  const prev = 'abc123';
  ...
  legacyEntry["hash"] = await sha(await legacy(prev, e, e.i, sha));
  ...
  first["hash"] = await sha(await legacy(GENESIS, e, e.i, sha));
...
console.log('\n=== §3 · it catches what an audit log exists to catch ===');
{
  ...
  ok((await chain.verify(truncated, { expectHead: log[3]["hash"] })).ok === false,
